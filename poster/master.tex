...
\block{Architecture}{
  \begin{description}
  \item[\numberItem{1}] \ l'utilisateur requiert sont API \\[.5em]
    \texttt{%
      % enums, commands = generate\_api(api, api\_number, profile) \\[.5em]
      enums, commands = generate\_api('gl', '4.0', 'core')% \colorG{}%
    }
  \item[\numberItem{2}] \ on construit le wrapper à la volé \\
    en exploitant l'XML API Registry \\
    et via un mécanisme de traduction automatisé de prototypes
    % \texttt{%
    % setattr(GL, enum, value) \\[.1em]
    % setattr(GL, command, CommandWrapper())%
    % }
  \item[\numberItem{3}] \ on code son application
  \end{description}
  \vspace{1cm}
  \begin{center}
    \begin{tikzpicture}[]
    ...
    \begin{tabular}[t]{p{18cm}p{18cm}}
      \begin{minipage}[t]{17cm}
        ...
        \item OpenGL Viewer \\
          ...
          \fbox{\includegraphics[width=8em]{images/font-map.png}}
        \end{itemize}
      \end{minipage}
      & 
      \begin{center}
  ...
  \begin{center}
    %\begin{tabular}{cc}
      \begin{minipage}{30cm}
        % \homePageOrange
        \textbf{Code Source~:} \\
        \url{https://github.com/FabriceSalvaire/PyOpenGLng} \\[1em]
        %
        ...
        \item Gestionnaire de PDF \\
          \url{https://github.com/FabriceSalvaire/Biblio}
        \end{itemize}
      \end{minipage}
      &
